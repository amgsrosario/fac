...
- `ADMINISTRADOR`: todas as permissões, incluindo anulação e auditoria.
- `OPERADOR`: consulta, criação/edição/eliminação de rascunhos, emissão, PDF e consulta de séries; não pode anular.
- `CONSULTA`: consulta documental, PDF e consulta de séries.

As permissões são derivadas do papel no login e incluídas no JWT. Alterações de papel exigem novo login/renovação do token. A migração atribui `ADMINISTRADOR` aos utilizadores existentes para não quebrar o ambiente atual; esta atribuição deve ser revista antes de produção.
